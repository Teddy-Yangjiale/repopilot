...
class Settings(BaseModel):
    ...
    def resolve_state_dir(self) -> Path:
        # Resolve relative to the current working directory so reports/tasks land where
        # the user runs the command, regardless of whether the package was pip-installed
        # (site-packages) or editable-installed (a source checkout).
        path = self.state_dir
